...
public final class LongDistanceWireRenderer<T extends BlockEntity & LongDistanceWireHost>
    ...
    @Override
    public AABB getRenderBoundingBox(T endpoint) {
        if (endpoint instanceof ElectricPoleBlockEntity pole) {
            return ElectricPoleBlockEntity.renderBounds(pole.getBlockPos(), pole.longDistance().clientConnections());
        }
        if (endpoint instanceof ElectricConnectorBlockEntity connector) {
            return ElectricConnectorBlockEntity.renderBounds(connector.getBlockPos(), connector.longDistance().clientConnections());
        }
        return BlockEntityRenderer.super.getRenderBoundingBox(endpoint);
    }
