...
public final class NuclearFacilityPortBlockEntity extends MachineBlockEntity {
    ...
    public <T> LazyOptional<T> getCapability(Capability<T> capability, @Nullable Direction side) {
        if (capability == ForgeCapabilities.ITEM_HANDLER
                && role != NuclearFacilityPartRole.ELECTRICAL
                && controllerPosition != null
                && side == outwardFacing()) {
    ...
    private void refreshItemCapability() {
        itemCapability.invalidate();
        reviveItemCapability();
    }

    private void reviveItemCapability() {
        itemCapability = role == NuclearFacilityPartRole.ELECTRICAL
                ? LazyOptional.empty()
                : LazyOptional.of(ForwardingItemHandler::new);
    }
