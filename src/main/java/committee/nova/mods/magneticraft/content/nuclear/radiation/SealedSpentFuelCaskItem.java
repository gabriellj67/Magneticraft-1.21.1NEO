...
public final class SealedSpentFuelCaskItem extends Item {
    ...
    public Optional<FuelAssemblyState> fuel(ItemStack stack) {
        return stack.hasTag() && stack.getTag().contains(FUEL_TAG)
                ? FuelAssemblyState.load(stack.getTagElement(FUEL_TAG)) : Optional.empty();
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide) {
            cask.addTagElement(FUEL_TAG, state.orElseThrow().save());
