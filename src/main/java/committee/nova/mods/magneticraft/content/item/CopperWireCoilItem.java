...
public final class CopperWireCoilItem extends Item {
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (player.isShiftKeyDown()) {
            WireSelectionPayload.Selection selection =
                    new WireSelectionPayload.Selection(serverLevel.dimension().location(), endpoint.get());
            CustomData.update(DataComponents.CUSTOM_DATA, stack, tag -> WireSelectionPayload.write(tag, selection));
        ...
        CompoundTag owner = stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag();
        Optional<WireSelectionPayload.Selection> selection = WireSelectionPayload.read(owner);
        ...
        if (attempt.result() == LongDistanceElectricityService.ConnectionResult.SUCCESS) {
            CustomData.update(DataComponents.CUSTOM_DATA, stack, WireSelectionPayload::clear);
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide) {
            CustomData owner = stack.get(DataComponents.CUSTOM_DATA);
            if (owner != null) {
                CompoundTag tag = owner.copyTag();
                WireSelectionPayload.clear(tag);
                if (tag.isEmpty()) {
