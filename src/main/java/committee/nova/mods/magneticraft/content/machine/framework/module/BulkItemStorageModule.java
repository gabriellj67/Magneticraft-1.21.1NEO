...
public final class BulkItemStorageModule implements MachineModule, IItemHandler {
    ...
    public void load(CompoundTag tag) {
        ItemStack loadedType = tag.contains(TYPE_TAG, CompoundTag.TAG_COMPOUND)
                ? ItemStack.of(tag.getCompound(TYPE_TAG))
                : ItemStack.EMPTY;
    ...
    public void save(CompoundTag tag) {
        if (!storedType.isEmpty() && amount > 0) {
            tag.put(TYPE_TAG, storedType.save(new CompoundTag()));
    ...
    public ItemStack insertItem(int slot, ItemStack stack, boolean simulate) {
        checkSlot(slot);
        if (stack.isEmpty()
                || (!storedType.isEmpty() && !ItemStack.isSameItemSameTags(storedType, stack))) {
            return stack;
        }
    ...
    public boolean isItemValid(int slot, ItemStack stack) {
        checkSlot(slot);
        return storedType.isEmpty() || ItemStack.isSameItemSameTags(storedType, stack);
    }
