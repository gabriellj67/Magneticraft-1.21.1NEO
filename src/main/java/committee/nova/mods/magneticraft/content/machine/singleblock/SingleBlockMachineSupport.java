...
final class SingleBlockMachineSupport {
    ...
    static Optional<SluiceRecipe> findSluiceRecipe(SingleBlockMachineBlockEntity machine, ItemStack stack) {
        ...
        return machine.getLevel().getRecipeManager().getRecipeFor(
                ...
                new SingleRecipeInput(stack.copyWithCount(1)),
                machine.getLevel()
    ...
    static IItemHandler adjacentItemHandler(SingleBlockMachineBlockEntity machine, Direction direction) {
        ...
        return Capabilities.ItemHandler.BLOCK.getCapability(
                level, targetPosition, level.getBlockState(targetPosition), blockEntity, direction.getOpposite()
        );
    }
    ...
    static int pushFluid(
        ...
        IFluidHandler target = Capabilities.FluidHandler.BLOCK.getCapability(
                level, targetPosition, level.getBlockState(targetPosition), targetEntity, direction.getOpposite()
        );
        if (target == null) {
            return 0;
        }
    ...
    private static boolean matchesFilter(SingleBlockMachineState state, ItemStack filter, ItemStack stack) {
        ...
        return !state.inserterUseNbt || Objects.equals(filter.getComponentsPatch(), stack.getComponentsPatch());
    }
}
