...
public final class CrushingTableBlockEntity extends MachineBlockEntity {
    ...
    public boolean interact(Player player, InteractionHand hand) {
        ...
        if (held.getItem() instanceof HammerItem hammer) {
            Optional<RecipeHolder<CrushingRecipe>> recipe = findRecipe(storedItem());
            if (recipe.isEmpty()) {
                returnStoredItem(player);
                return true;
            }
            if (recipe.get().value().requiredLevel() > hammer.type().crushingLevel()) {
                return true;
            }

            ItemStack input = storedItem().copyWithCount(1);
            progress += hammer.type().crushingSpeed();
            held.hurtAndBreak(hammer.type().crushingDurabilityCost(), player, EquipmentSlot.MAINHAND);

            if (progress >= REQUIRED_PROGRESS) {
                lastItem = input;
                inventory.setStackInSlot(0, recipe.get().value().output());
                progress = 0;
    ...
    protected void saveMachineData(CompoundTag tag, HolderLookup.Provider registries) {
        ...
        if (!lastItem.isEmpty()) {
            tag.put(LAST_ITEM_TAG, lastItem.save(registries));
        }
    }
    ...
    protected void loadMachineData(CompoundTag tag, HolderLookup.Provider registries) {
        ...
        lastItem = tag.contains(LAST_ITEM_TAG) ? ItemStack.parseOptional(registries, tag.getCompound(LAST_ITEM_TAG)) : ItemStack.EMPTY;
        if (!lastItem.isEmpty()) {
            lastItem.setCount(1);
        }
    }
    ...
    protected void saveClientData(CompoundTag tag, HolderLookup.Provider registries) {
        tag.putInt(PROGRESS_TAG, progress);
        if (!storedItem().isEmpty()) {
            tag.put(DISPLAY_ITEM_TAG, storedItem().save(registries));
        }
    }
    ...
    protected void loadClientData(CompoundTag tag, HolderLookup.Provider registries) {
        ...
        inventory.setStackInSlot(
                0,
                tag.contains(DISPLAY_ITEM_TAG) ? ItemStack.parseOptional(registries, tag.getCompound(DISPLAY_ITEM_TAG)) : ItemStack.EMPTY
        );
    }
    ...
    private boolean refillLastInput(Player player) {
        ...
        for (int slot = 0; slot < playerInventory.getContainerSize(); slot++) {
            ...
            if (!candidate.isEmpty() && ItemStack.isSameItemSameComponents(candidate, lastItem) && findRecipe(candidate).isPresent()) {
    ...
    private Optional<RecipeHolder<CrushingRecipe>> findRecipe(ItemStack input) {
        Level currentLevel = getLevel();
        if (currentLevel == null || input.isEmpty()) {
            return Optional.empty();
        }
        SingleRecipeInput recipeInput = new SingleRecipeInput(input.copyWithCount(1));
        return currentLevel.getRecipeManager().getRecipeFor(ModRecipeTypes.CRUSHING_TYPE.get(), recipeInput, currentLevel);
    }
}
