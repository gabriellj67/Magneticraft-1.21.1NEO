        ...
        public boolean accepts(FluidStack stack, @Nullable ServerLevel level) {
            ...
            for (String accepted : acceptedFluids) {
                ...
                if (COMBUSTION_FUEL_RECIPE.equals(accepted) && level != null && level.getRecipeManager()
                        ...
                        .stream()
                        .anyMatch(recipe -> recipe.value().fluid() == stack.getFluid())) {
                    return true;
                }
                if (BuiltInRegistries.FLUID.getKey(stack.getFluid()) != null
                        && accepted.equals(BuiltInRegistries.FLUID.getKey(stack.getFluid()).toString())) {
                    return true;
                }
            }
            return false;
