...
public final class MagneticraftJeiPlugin implements IModPlugin {
    /**
     * Replaces Forge JEI's {@code useNbtForSubtypes} (removed - NBT-based item data is largely gone,
     * see the vanilla data-component rewrite). Any non-default component data is treated as
     * subtype-distinguishing, the same effect the old "use NBT" shortcut had.
     */
    private static final ISubtypeInterpreter<ItemStack> COMPONENT_PATCH_INTERPRETER =
    ...
    /**
     * Kept on the vanilla recipe's own type/id via {@link RecipeHolder} (unlike the other types
     * above, which mint their own JEI-only id): the "route" label in
     * {@link NuclearProcessingRecipeCategory} depends on the recipe's resource id, which no
     * longer lives on the {@code Recipe} object itself (Phase 4's recipe rewrite) - only the
     * {@link RecipeHolder} the recipe manager returns still carries it.
     */
    public static final RecipeType<RecipeHolder<NuclearProcessRecipe>> NUCLEAR_PROCESSING =
            RecipeType.createFromVanilla(ModRecipeTypes.NUCLEAR_PROCESSING_TYPE.get());
    ...
    public void registerItemSubtypes(ISubtypeRegistration registration) {
        for (Item item : List.of(
            ...
            registration.registerSubtypeInterpreter(item, COMPONENT_PATCH_INTERPRETER);
        }
    }
    ...
    public void registerRecipes(IRecipeRegistration registration) {
        ...
        registration.addRecipes(CRUSHING, values(recipes.getAllRecipesFor(ModRecipeTypes.CRUSHING_TYPE.get())));
        registration.addRecipes(SLUICE, values(recipes.getAllRecipesFor(ModRecipeTypes.SLUICE_TYPE.get())));
        registration.addRecipes(GASIFICATION, values(recipes.getAllRecipesFor(ModRecipeTypes.GASIFICATION_TYPE.get())));
        registration.addRecipes(THERMOPILE, values(recipes.getAllRecipesFor(ModRecipeTypes.THERMOPILE_TYPE.get())));
        registration.addRecipes(FLUID_FUEL, values(recipes.getAllRecipesFor(ModRecipeTypes.FLUID_FUEL_TYPE.get())));
        registration.addRecipes(POLYMERIZING, values(recipes.getAllRecipesFor(ModRecipeTypes.POLYMERIZING_TYPE.get())));
    ...
    private static <T extends Recipe<?>> List<T> values(List<RecipeHolder<T>> holders) {
